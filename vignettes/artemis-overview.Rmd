...
models for qPCR data from eDNA samples. eDNA data has often been analysed with occupancy models or GLMS, but there are
several characteristics of eDNA qPCR data which made us feel that it would
benefit from a different modeling approach.  Specifically, truncated latent variable models written in [Stan](mcmc-stan.org) and included with `artemis`.
...
The primary purpose of the `artemis` package is to fit the model to
...
We provide a model formula, a `dataframe` or `list` of the observed
data, and the parameters for the associated standard curve equation to convert
