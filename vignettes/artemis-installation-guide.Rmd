...
<br>
The easiest way to install `artemis` is from [CRAN](https://cran.r-project.org/web/packages/artemis/index.html) with the `install.packages` function:
...
model_fit = eDNA_lm(Cq ~ Distance_m + Volume_mL + Distance_m*Volume_mL, 
                    data = eDNA_data,
