...
sampling and qPCR data. There are several reasons we felt that
modeling of eDNA data could benefit from this treatment:
  ...
  1. In eDNA samples, the concentration of eDNA is not directly
     ...
     target species. This standard curve formula typically takes the form: 
	 $$Cq = log[eDNA] * \beta + \alpha$$
	 
  2. A lower Cq value corresponds to a higher concentration of eDNA in
     ...
     than the threshold number of cycles to detect.  This data
     truncation in the observed response increases uncertainty in our
     estimate of the underlying response variable, eDNA concentration.
...
latent (unobserved) eDNA concentration, linking it to the truncated
observed response via a standard curve, and estimating the measurement
error in the process. The general model is as follows:

$$ Cq_i \sim Normal(\hat{Cq_i}, \sigma_{Cq}) \\
\hat{Cq_i} = \alpha_{std\_curve} + \beta_{std\_curve}* log[eDNA]_i  \\
log[eDNA]_{i} = X_{i} \beta$$

where $\beta$ is a vector of effects on $log[eDNA]$, and
...
Because `artemis` was originally written to analyze aquatic eDNA
samples, the examples and sample data included in the package have
variables that are specific to aquatic eDNA survey data, and even more
specifically to "live car" experiments, where eDNA is sampled at known
distances from a source of eDNA (fish in a net pen in unidirectional
flow, for example).  These variables include `volume` (the the number
of mililiters of water that went through the filter in each sample)
and `distance` (the distance in meters from the source of eDNA).
However, the functions in `artemis` do generalize to any eDNA survey
data, so long as that study generates Cq values and is associated with
a standard curve for the target species.
...
variable in your survey data.  In the following example, our variables
are `distance`, `volume`, `tech_rep`, and `rep`.
...
Notice that we also provided a level for the intercept, as well as a
dummy variable for the response. The response "level" is not used
except internally to create the model matrix.  By default, the
simulation functions construct a model matrix assuming a complete
block design (i.e. every level of each variable is represented). The
simulation function expands the variable list using
`base::expand.grid()`.
...
It is important to note that these are the effect sizes on
$log[eDNA]$, *not* on the Cq values themselves. All the parameters of
the model are on the log scale, including the intercept. Hence, we
specified the intercept to be -10, which corresponds to
exp(-10)$[eDNA]$. In practice, if the default value for log[eDNA] is
close to 0, some a negative number less than -10 is a fair assumption
for the intercept beta.
